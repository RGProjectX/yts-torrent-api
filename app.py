...
def search_yts():
	...
	if data.status_code == int(200):
		...
		for movie in response['data']:
			try:
				...
				crew = [[
				    x.img['alt'].replace('Picture', '').strip(), x['href']
				] for x in soup]
				entry = \
                {
				 "title": movie['title'],
	...
	elif data.status_code != int(200):
		print(data.status_code)
		results = {
		    'status': "Error Loading API",
		    'possible_error': {
		        'web_block': 'Website Has Been Blocked',
		        'name_error': 'Please Check Movie Name'
		    }
		}
		results = sorted(
		    results, key=lambda entry: entry['rating'], reverse=True)
